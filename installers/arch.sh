#!/usr/bin/env bash
# installers/arch.sh — install an Arch Linux system from manifest
...
bootstrap_distro() {
  ...
  fetch_stage3 "arch" "rolling" "$arch" "$stage3"
  mkdir -p "$target"
  ...
  in_chroot "$target" pacman-key --init
  in_chroot "$target" pacman-key --populate archlinux
  in_chroot "$target" pacman -Sy --noconfirm archlinux-keyring
}
...
install_packages() {
  ...
  for pkg in $explicit_raw; do
    local translated; translated=$(pkgmap_canonical_to_distro "$pkg" "arch")
    [[ -n "$translated" ]] && pkgs+=("$translated")
  done
  [[ ${#pkgs[@]} -eq 0 ]] && return 0
  in_chroot "$target" pacman -S --noconfirm --needed "${pkgs[@]}" 2>/dev/null || true
}
...
main() {
  ...
  log_step "Installing Arch Linux from manifest"
