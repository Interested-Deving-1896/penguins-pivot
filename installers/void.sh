#!/usr/bin/env bash
# installers/void.sh — install a Void Linux system from manifest
...
bootstrap_distro() {
  ...
  fetch_stage3 "void" "rolling" "$arch" "$stage3"
  mkdir -p "$target"
  ...
  in_chroot "$target" xbps-install -Syu xbps
}
...
install_packages() {
  ...
  for pkg in $explicit_raw; do
    local translated; translated=$(pkgmap_canonical_to_distro "$pkg" "void")
    [[ -n "$translated" ]] && pkgs+=("$translated")
  done
  [[ ${#pkgs[@]} -eq 0 ]] && return 0
  in_chroot "$target" xbps-install -y "${pkgs[@]}" 2>/dev/null || true
}
...
main() {
  ...
  log_step "Installing Void Linux from manifest"
