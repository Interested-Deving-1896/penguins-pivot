#!/usr/bin/env bash
# installers/alpine.sh — install an Alpine Linux system from manifest
...
bootstrap_distro() {
  ...
  fetch_stage3 "alpine" "$release" "$arch" "$stage3"
  mkdir -p "$target"
...
install_packages() {
  ...
  for pkg in $explicit_raw; do
    local translated; translated=$(pkgmap_canonical_to_distro "$pkg" "alpine")
    [[ -n "$translated" ]] && pkgs+=("$translated")
  done
  [[ ${#pkgs[@]} -eq 0 ]] && return 0
  in_chroot "$target" apk add --no-cache "${pkgs[@]}" 2>/dev/null || true
}
...
main() {
  ...
  log_step "Installing Alpine Linux from manifest"
