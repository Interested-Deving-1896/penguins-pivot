#!/usr/bin/env bash
# installers/opensuse.sh — install an openSUSE system from manifest
...
bootstrap_distro() {
  ...
  fetch_stage3 "opensuse" "$release" "$arch" "$stage3"
  mkdir -p "$target"
  ...
  in_chroot "$target" zypper --non-interactive refresh
}
...
install_packages() {
  ...
  for pkg in $explicit_raw; do
    local translated; translated=$(pkgmap_canonical_to_distro "$pkg" "opensuse")
    [[ -n "$translated" ]] && pkgs+=("$translated")
  done
  [[ ${#pkgs[@]} -eq 0 ]] && return 0
  in_chroot "$target" zypper --non-interactive install "${pkgs[@]}" 2>/dev/null || true
}
...
main() {
  ...
  log_step "Installing openSUSE from manifest"
