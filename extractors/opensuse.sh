#!/usr/bin/env bash
# extractors/opensuse.sh — extract manifest from openSUSE
source "$(dirname "${BASH_SOURCE[0]}")/common.sh"

extract_packages() {
  local manifest="$1" root="${2:-/}"
  local all; all=$(chroot "$root" rpm -qa --qf '%{NAME}\n' 2>/dev/null | sort)
  local explicit; explicit=$(chroot "$root" zypper packages --installed-only 2>/dev/null | awk -F'|' 'NR>4 && /^i/{gsub(/ /,"",$3); print $3}' | sort || echo "$all")
...
main() {
  ...
  log_step "Extracting manifest from openSUSE"
